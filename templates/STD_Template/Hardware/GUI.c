...
void LCD_Printf(uint16_t X, uint16_t Y, uint16_t FC, uint16_t BC,uint8_t size, uint8_t mode, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	uint8_t XOffect=0,YOffect=0;
	for(uint16_t CHAR=0;CHAR<strlen(String);CHAR++)
	{	
		if(String[CHAR]!='\r' && String[CHAR]!='\n')
		{
		    LCD_ShowChar(X+XOffect,Y+YOffect,FC,BC,String[CHAR],size,mode);
			XOffect+=size/2;
		}
		else if(String[CHAR]=='\r' && String[CHAR+1]=='\n')
		{
			YOffect+=size;
			XOffect=0;
			CHAR++;
